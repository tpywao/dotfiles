...
- 各ディレクトリの `install.sh`（`git/`, `docker/`, `sheldon/`, `karabiner/`, `ghostty/`, `nix/`, `brew/`, `claude/`, `zsh/`）: そのディレクトリに関する処理
...
- ルートは `sh "$DOTFILES/<dir>/install.sh" || exit $?` で呼ぶ。サブ側の `exit 1`（`nix/install.sh` の `DOTFILES_MACHINE` 未設定など）はルートまで伝播する
