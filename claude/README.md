...
Claude Code の設定一式。`install.sh` の `link_claude_md` がこのディレクトリ配下のファイルを `~/.claude/` へ **hardlink** で同期する。ただし `settings.json` は hardlink せず、`merge_claude_settings` が共有キーだけを既存の設定へ上書き適用する。

## hardlink 同期の仕組み

- 初回リンクは `install.sh`（`claude/` 配下を再帰的に `~/.claude/` へ hardlink。`settings.json` は除外）
...
Claude Code 自身が `model` / `effortLevel` / `modelSettings` / `autoMode` を `~/.claude/settings.json` へ書き込むため、hardlink を張ると次の 2 つが起きる。

- マシン固有の値が dotfiles 側へ流れ込む。特に `autoMode.environment` にはホームディレクトリのパスやリポジトリ名が入る
- 書き込みが atomic save なのでリンクが切れる。しかも本体の設定書き込みは PostToolUse hook を通らないため `relink-claude-files.sh` では復旧できず、気づかないまま両側が別々に育つ
...
| `relink-claude-files.sh` | PostToolUse (Edit\|Write) | 上記の hardlink 再同期（`settings.json` は対象外） |
