...
worktree を作る前に、進行中の別作業と対象ファイルが衝突しないか確認する:

1. `git worktree list` — 別セッションが作業中の worktree（locked のものを含む）
2. `gh pr list` — オープンな PR
3. 衝突しそうなものがあれば差分で対象ファイルを見る（`gh pr diff <n> --name-only`、`git diff --stat origin/main...<branch>`）

衝突する場合は、相手のマージを待つか、そのブランチ側で一緒に対応する。
...
PR も同じ基準で分ける。障害対応・性能改善・リファクタリングが同時に必要になっても、目的が違うなら別 PR にする。

- 分割の基準: 「その PR 単体で revert できるか」
- 分けられない場合は、PR 本文に revert 単位（どのコミットがどの目的か）を明記する
- 理由: 目的が混在した PR では、レビュアーが「必須の変更」と「任意の変更」の切り分けを自力でやることになり、レビュー負荷が跳ね上がる
...
### ドットファイルを数えるときは ls を使わない

`ls` と `*` グロブはドットで始まるファイルを含めない。ディレクトリの中身を数える・一覧する目的では `ls -A` か `find` を使う。

理由: ドットファイルだけが入ったディレクトリを「0 件」と誤判定する。実際に `~/.claude/backups/`（`.claude.json.backup.*` が並ぶ）を空だと報告した。

### git -C を使わない

`git -C <パス>` は使わない。カレントディレクトリが対象リポジトリ内なら素の `git`、cwd の外のリポジトリ（worktree や別リポジトリ）を操作するときは `cd <パス> && git <サブコマンド>` で実行する。

理由: 許可ルールは複合コマンドを `&&` `||` `;` `|` `|&` `&` 改行で分割し、各サブコマンドを独立に照合する。`cd <パス> && git log` は `cd`（常時自動許可）と `git log`（読み取り系として自動許可、`Bash(git log *)` にも合致）がそれぞれマッチするためプロンプトが出ない。`git -C <パス> log` は 1 個のコマンド文字列全体が `Bash(git log *)` に一致せずプロンプトになる。

`Bash(git -C * log *)` のような許可ルールで解決しない。`*` は空白を含む任意テキストにマッチするため、`-c` や `--exec-path` を差し込んだ任意コード実行まで無確認で通る。Claude Code もサブコマンドより前に `*` を置いた allow ルールを起動時に警告する。
...
理由: 着手後に main が進んでいると、HEAD だけが最新の main へ動いて作業ツリーは古いままになり、差分が「その間にマージされた変更を巻き戻すもの」になる。気づかずコミットすると他人の変更を打ち消す（実際に、削除済みのファイルを復活させかけた）。

やってしまったら、自分の変更を退避 → `git checkout -- .` で作業ツリーを HEAD に合わせる → 退避分を当て直す、の順で戻す。
...
フラグの判定はいずれも `[^|;&]*` で同一サブコマンド内に限定してある。`shellcheck -f gcc ...; git push` のように、無関係なコマンドの `-f` が禁止コマンドと同居しただけではブロックされない。
